...
public final class SKDRequestDictionary {
    ...
    @inlinable
    public subscript(key: KeyPath<sourcekitd_keys, sourcekitd_uid_t>) -> String {
        get { self[sourcekitd.keys[keyPath: key]] }
        set { self[sourcekitd.keys[keyPath: key]] = newValue }
    }
    
    @inlinable
    public subscript(key: KeyPath<sourcekitd_keys, sourcekitd_uid_t>) -> Int {
        get { self[sourcekitd.keys[keyPath: key]] }
        set { self[sourcekitd.keys[keyPath: key]] = newValue }
    }

    @inlinable
    public subscript(key: KeyPath<sourcekitd_keys, sourcekitd_uid_t>) -> sourcekitd_uid_t? {
        get { self[sourcekitd.keys[keyPath: key]] }
        set { self[sourcekitd.keys[keyPath: key]] = newValue }
    }

    @inlinable
    public subscript(key: KeyPath<sourcekitd_keys, sourcekitd_uid_t>) -> SKDRequestDictionary {
        get { self[sourcekitd.keys[keyPath: key]] }
        set { self[sourcekitd.keys[keyPath: key]] = newValue }
    }

    @inlinable
    public subscript<S>(key: KeyPath<sourcekitd_keys, sourcekitd_uid_t>) -> S where S: Sequence, S.Element == String {
        get { self[sourcekitd.keys[keyPath: key]] }
        set { self[sourcekitd.keys[keyPath: key]] = newValue }
    }

    @inlinable
    public subscript(key: KeyPath<sourcekitd_keys, sourcekitd_uid_t>) -> SKDRequestArray {
        get { self[sourcekitd.keys[keyPath: key]] }
        set { self[sourcekitd.keys[keyPath: key]] = newValue }
    }
}
